...
7. **[Act](https://github.com/nektos/act) (optional):** Required to run the
   GitHub Actions build and E2E workflow locally.
...
Use Act to run the `testbuild` job from `bake_targets.yml` directly from the
repository root on Linux. These instructions were tested with the `plr`
extension. Local mode pushes images to a local registry on the shared
`pg-extensions-e2e` Docker network and exercises image and SBOM generation.

Start the local registry and E2E network before running the workflow:

```bash
task e2e:start-container-registry
```

Run the build for one extension and one architecture. Replace `<extension>`
with a target such as `pgvector`:

```bash
artifact_dir="$(mktemp -d)"
act workflow_call \
  -j testbuild \
  -W .github/workflows/bake_targets.yml \
  --input extension_name=<extension> \
  --input local=true \
  --input distro=bookworm \
  --input platform=linux/amd64 \
  --network pg-extensions-e2e \
  -P ubuntu-24.04=catthehacker/ubuntu:act-latest \
  --container-options '--privileged' \
  --artifact-server-path "$artifact_dir"
```

Omit either `--input distro=bookworm` or `--input platform=linux/amd64` to use
the default matrix for that dimension. The default builds both Debian distros
and both supported architectures.
The build output lists the exact local image references, for example
`registry.pg-extensions:5000/pgvector-testing:<tag>`.

The `-j testbuild` selection is intentional: the downstream smoke and security
jobs use matrices generated by earlier jobs, which the current Act release
cannot expand.

When finished, remove the local E2E resources and temporary artifact files:
