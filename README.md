...
fields and into the owning packages' `licenseInfoFromFiles` fields. For
compatibility with scanners such as Trivy, those file findings also
provisionally populate an owning package's `licenseDeclared` field only when
that field is missing or `NOASSERTION`. Existing package declarations are
preserved. This is a scanner-oriented license inventory projection, not a
definitive statement of the package authors' legally declared license; the
file-level fields remain the detailed evidence and `licenseConcluded` is
unchanged.

The original builder SBOM is generated by BuildKit's SBOM attestation using
Anchore's Syft scanner. The current builder predicate records `Tool:
syft-v1.51.0`; this version is supplied by the BuildKit builder and may change
when the builder is updated. Syft's Debian package cataloger incorrectly leaves
many package license fields as `NOASSERTION` even when Debian copyright files
are present. This limitation is tracked upstream in [Syft #2861](https://github.com/anchore/syft/issues/2861)
and [Syft #2724](https://github.com/anchore/syft/issues/2724). The ScanCode
pass above is a provisional supplement for this reason.

Debian binary package control metadata does not provide a standard license
field that can be used as a fallback. An audit of all 17 direct PostgreSQL 18
extension packages in both bookworm and trixie found no `License`, `X-License`,
or `SPDX` field in either the package indexes or the downloaded `.deb` control
archives. Debian normally distributes this information separately in each
package's `/usr/share/doc/<package>/copyright` file.
